...
export default function deleteResources(res, delData) {
    var _a, _b;
    ...
    for (const d of delData) {
        ...
        for (let i = res.entries.length - 1; i >= 0; --i) {
            ...
            found = true;
            log.debug(`[delete] Delete resource entry (type: ${entry.type}, id: ${entry.id}, lang: ${entry.lang})`);
            res.entries.splice(i, 1);
        }
        if (!found && d.failIfNoDelete) {
            throw new Error(`Resource is not found: type = ${d.type}, id = ${(_a = d.id) !== null && _a !== void 0 ? _a : '(not specified)'}, lang = ${(_b = d.lang) !== null && _b !== void 0 ? _b : '(not specified)'}`);
        }
    }
