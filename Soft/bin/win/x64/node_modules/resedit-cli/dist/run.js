...
import { CertificateSelectMode, certificateSelectModeValues, PredefinedResourceTypeName, PredefinedResourceTypeNameForDelete, } from './definitions/DefinitionData.js';
...
function convertOptionsToDefinitionData(outDefinition, options) {
    ...
    if (options.raw2 && options.raw2.length > 0) {
        if (outDefinition.raw && outDefinition.raw.length > 0) {
            log.debug(`Append raw resource definitions with ones from option '--raw2'. (count = ${options.raw2.length})`);
        }
        else {
            log.debug(`Add raw resource definitions from option '--raw2'. (count = ${options.raw2.length})`);
        }
        outDefinition.raw = ((_c = outDefinition.raw) !== null && _c !== void 0 ? _c : []).concat(options.raw2.map((value, i) => {
            const ra = /^([^,]+?),([^,]+?),(.+)$/.exec(value);
            if (!ra) {
                throw new Error(`Invalid '--raw2' option value (index: ${i}, expected: <typeName>,<ID>,<data>, actual: ${value})`);
            }
            const typeName = ra[1];
            if (!Object.keys(PredefinedResourceTypeName).includes(ra[1])) {
                throw new Error(`Invalid '--raw2' option value (index: ${i}, <typeName> is not valid; actual: ${value})`);
            }
            const type = PredefinedResourceTypeName[typeName];
            const id = convertIntegerOrStringValue(ra[2]);
            if (ra[3][0] === '@') {
                return {
                    type,
    ...
    if (options.delete) {
        if (outDefinition.delete && outDefinition.delete.length > 0) {
            log.debug(`Append delete resource definitions with ones from option. (count = ${options.delete.length})`);
        }
        else {
            log.debug(`Add delete resource definitions from option. (count = ${options.delete.length})`);
        }
        outDefinition.delete = ((_d = outDefinition.delete) !== null && _d !== void 0 ? _d : []).concat(options.delete.map((value, i) => {
            const ra = /^([^,]+?)(?:,(.+))?$/.exec(value);
            if (!ra) {
                throw new Error(`Invalid '--delete' option value (index: ${i}, expected: <type>,<ID>, actual: ${value})`);
            }
            const type = convertIntegerOrStringValue(ra[1]);
            if (ra[2] == null) {
                return { type };
    ...
    Object.keys(PredefinedResourceTypeNameForDelete).forEach((_key) => {
        ...
        if (optBase) {
            ...
            if (outDefinition.delete && outDefinition.delete.length > 0) {
                log.debug(`Append delete resource definitions with ones from option [--delete-${key}]. (count = ${opt.length})`);
            }
            else {
                log.debug(`Add delete resource definitions from option [--delete-${key}]. (count = ${opt.length})`);
            }
            const type = PredefinedResourceTypeNameForDelete[key];
            outDefinition.delete = ((_a = outDefinition.delete) !== null && _a !== void 0 ? _a : []).concat(opt
                .map((value) => {
