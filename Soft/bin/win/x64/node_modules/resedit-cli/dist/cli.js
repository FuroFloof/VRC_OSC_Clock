...
import { certificateSelectModeValues, PredefinedResourceTypeName, PredefinedResourceTypeNameForDelete, } from './definitions/DefinitionData.js';
import { getValidDigestAlgorithm } from './definitions/parser/sign.js';
const require = createRequire(import.meta.url);
const loglevel = require('loglevel');
const yargsFactory = require('yargs');
...
async function main() {
  ...
  ${thisName} [[--in] <in> | --new] [--out] <out> [<options...>]`)
            .option('allow-shrink', {
            description: 'Allow shrinking resource section size (if the data size is less than original)',
            type: 'boolean',
            nargs: 0,
        })
            .option('as-32bit', {
            type: 'boolean',
            description: 'Creates the executable binary as a 32-bit version (default: as 64-bit).\nRequires --new option.',
            nargs: 0,
        })
            .option('as-exe-file', {
            type: 'boolean',
            description: 'Creates the executable binary as an EXE file (default: as a DLL).\nRequires --new option.',
            nargs: 0,
        })
            .option('certificate', {
            alias: ['cert'],
            description: 'Certificate file for signing.\n(DER format file (.cer) or PEM format file (.pem) is supported)\nRequires --sign option.',
            type: 'string',
            nargs: 1,
        })
            .option('company-name', {
            ...
            .option('debug', {
            type: 'boolean',
            description: 'Output more logs than verbose mode while processing.',
            nargs: 0,
        })
            .option('delete', {
            description: 'One or more resources to delete from executable.\nThe value must be one of following format:\n* <type>,<ID>\n* <type>',
            type: 'array',
            nargs: 1,
        })
            .option('delete-xxxxx', {
            description: 'One or more resources to delete from executable (xxxxx is one of the predefined type names described below).\nThe value must be <ID> or no value.',
            type: 'array',
            requiresArg: false,
        })
            .options(Object.keys(PredefinedResourceTypeNameForDelete).reduce((prev, _key) => {
            const key = _key;
            prev[`delete-${key}`] = {
            ...
            .option('definition', {
            description: 'Resource definition file which contains resource data to write (see document for details)',
            type: 'string',
            nargs: 1,
        })
            .option('digest', {
            description: "Digest algorithm for signing. (default: 'sha256')\nRequires --sign option.",
            type: 'string',
            choices: getValidDigestAlgorithm(),
            nargs: 1,
        })
            .option('fail-if-no-delete', {
            description: 'If specified and the resource to be deleted does not exist, the operation will fail.\nIf no delete options are specified, this flag is ignored.',
            type: 'boolean',
            nargs: 0,
        })
            ...
            .option('file-version', {
            description: "File version for version resource.\nMust be 'n.n.n.n' format (n is an integer)",
            type: 'string',
            nargs: 1,
        })
            .option('grow', {
            ...
            .option('icon', {
            description: 'One or more icon files to add to executable.\nIcon ID can be specified with following format: <ID>,<file-name>',
            type: 'array',
            nargs: 1,
        })
            .option('ignore-signed', {
            description: 'Force read input file even if it is a signed executable',
            type: 'boolean',
        })
            .option('in', {
            alias: 'i',
            ...
            alias: ['n'],
            type: 'boolean',
            description: 'Create an empty (data-only) executable binary.\nCannot specify an input file if this option is used.',
            nargs: 0,
        })
            .option('no-grow', {
            alias: ['N'],
            description: 'Disallow growing resource section size (throw errors if data exceeds)',
            type: 'boolean',
            nargs: 0,
        })
            ...
            .option('p12', {
            alias: ['pfx'],
            description: 'PKCS 12 file (.p12 or .pfx file), which contains private key and certificates, for signing.\nRequires --sign option.',
            type: 'string',
            nargs: 1,
        })
            .option('password', {
            description: 'Password/passphrase for private key. If an empty string password is required, specify \'\' (sh) or "" (cmd.exe).\nRequires --sign option.',
            type: 'string',
            nargs: 1,
        })
            .option('private-key', {
            alias: ['key'],
            description: 'Private key file for signing. (only PEM format file (.pem) is supported)\nRequires --sign option.',
            type: 'string',
            nargs: 1,
        })
            .option('product-name', {
            ...
            description: "Product version for version resource.\nMust be 'n.n.n.n' format (n is an integer)",
            type: 'string',
            nargs: 1,
        })
            .option('raw', {
            description: 'One or more resources to add to executable.\nThe value must be one of following format:\n* <type>,<ID>,<string-value>\n* <type>,<ID>,@<file-name>\n(<string-value> will be stored as UTF-8 string)',
            type: 'array',
            nargs: 1,
        })
            .option('raw2', {
            ...
            .epilogue(`The predefined type names are: ${Object.keys(PredefinedResourceTypeName)
            ...
            if (argv.in == null || argv.in === '') {
                ...
                throw new CommandLineError('cannot specify both input file and --new.');
            }
            if (argv.out === '') {
                throw new CommandLineError('output file is missing.');
            }
            if (argv.new && argv.grow === false) {
                throw new CommandLineError('--no-grow cannot be used with --new');
            }
            if (restArgs.length > 0) {
                throw new CommandLineError('Unknown arguments: ' + restArgs.join(', '));
            }
            return true;
        })
            .fail((msg, err, yargs) => {
        ...
        if (s instanceof CommandLineError) {
            ...
            console.error(`${thisName}:`, (_a = s.stack) !== null && _a !== void 0 ? _a : s.message);
        }
        else {
            let msg;
            if (typeof s !== 'string') {
